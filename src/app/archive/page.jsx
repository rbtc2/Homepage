...
export default async function ArchivePage({ searchParams }) {
  ...
  if (isSearching) {
    const matched = (await searchArchives(query)).sort((a, b) => b.id - a.id);
    ...
    const sorted = [...allArchives].sort((a, b) => b.id - a.id);
          ...
          <div className="notice-board__inner">
            ...
            <div className="notice-board__meta">
              {isSearching ? (
                ...
                <span className="notice-board__count">
                  전체 <strong>{allArchives.length}</strong>건
                </span>
              )}
            </div>
            ...
            <table className="notice-table">
              ...
              <tbody>
                {rows.length === 0 ? (
                  <tr>
                    <td className="notice-table__empty" colSpan={5}>
                      ...
                      <p className="notice-table__empty-desc">
                        ...
                        <br />
                        다른 검색어를 입력하거나{' '}
                        <Link href="/archive" className="notice-table__empty-link">
                          전체 목록
                        </Link>
                        을 확인하세요.
                      </p>
                  ...
                  rows.map((archive) => (
                    <tr key={archive.id} className="notice-table__row">
                      <td className="notice-table__td notice-table__td--num">{archive.rowNum}</td>
                      <td className="notice-table__td notice-table__td--title">
                        <Link href={`/archive/${archive.id}`} className="notice-table__link">
                          {isSearching ? (
                            <HighlightedText text={archive.title} query={query} />
                          ) : (
                            archive.title
                          )}
                        </Link>
                      </td>
                      <td className="notice-table__td notice-table__td--author">{archive.author}</td>
                      <td className="notice-table__td notice-table__td--date">{archive.createdAt}</td>
                      <td className="notice-table__td notice-table__td--views">
