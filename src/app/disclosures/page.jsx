...
function buildHref(page, query) {
  ...
  return qs ? `/disclosures?${qs}` : '/disclosures';
}
...
export default async function DisclosuresPage({ searchParams }) {
  ...
  if (isSearching) {
    const matched = (await searchDisclosures(query)).sort((a, b) => b.id - a.id);
    ...
    const sorted = [...allDisclosures].sort((a, b) => b.id - a.id);
            ...
            <div className="notice-board__meta">
              {isSearching ? (
                <span className="notice-board__count">
                  ...
                  <Link href="/disclosures" className="notice-board__clear" aria-label="검색 초기화">
                ...
                <span className="notice-board__count">
                  전체 <strong>{allDisclosures.length}</strong>건
                </span>
              )}
            </div>
            ...
            <table className="notice-table">
              ...
              <tbody>
                {rows.length === 0 ? (
                  <tr>
                    <td className="notice-table__empty" colSpan={5}>
                      ...
                      <p className="notice-table__empty-desc">
                        &ldquo;{query}&rdquo;에 해당하는 공시자료를 찾지 못했습니다.
                        <br />
                        다른 검색어를 입력하거나{' '}
                        <Link href="/disclosures" className="notice-table__empty-link">
                          전체 목록
                        </Link>
                        을 확인하세요.
                      </p>
                  ...
                  rows.map((d) => (
                    <tr key={d.id} className="notice-table__row">
                      <td className="notice-table__td notice-table__td--num">{d.rowNum}</td>
                      <td className="notice-table__td notice-table__td--title">
                        <Link href={`/disclosures/${d.id}`} className="notice-table__link">
                          {isSearching ? <HighlightedText text={d.title} query={query} /> : d.title}
                        </Link>
                      </td>
                      <td className="notice-table__td notice-table__td--author">{d.author}</td>
                      <td className="notice-table__td notice-table__td--date">{d.createdAt}</td>
                      <td className="notice-table__td notice-table__td--views">{d.views.toLocaleString()}</td>
                    </tr>
                  ))
                )}
              </tbody>
