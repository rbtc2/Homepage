...
import { updateNoticeEnglish } from './actions';

export default function NoticesEnglishEditorPage({ post }) {
  ...
  const handleSave = async ({ title, content }) => {
    const result = await updateNoticeEnglish(post.id, { title, content });
  ...
  return (
    <RichEditor
      ...
      editTitle={hasEnglish ? '공지사항 영문 수정' : '공지사항 영문 작성'}
      newTitle="공지사항 영문 작성"
      showMetaDate={false}
      ...
      notice={
        <>
          <p>작성일·공지 고정·비밀글 설정은 한국어 글과 같습니다. 제목과 본문만 영어로 작성하세요.</p>
