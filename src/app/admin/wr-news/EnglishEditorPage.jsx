...
import { updateWrNewsEnglish } from './actions';

export default function WrNewsEnglishEditorPage({ post }) {
  ...
  const handleSave = async ({ title, content }) => {
    const result = await updateWrNewsEnglish(post.id, { title, content });
  ...
  return (
    <RichEditor
      ...
      editTitle={hasEnglish ? 'WR뉴스 영문 수정' : 'WR뉴스 영문 작성'}
      newTitle="WR뉴스 영문 작성"
      showMetaDate={false}
